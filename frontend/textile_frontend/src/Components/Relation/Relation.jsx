...
import ReportRelationView from "./ReportRelationView";
...
export default function Relation() {
  ...
  const loadAll = async () => {
    try {
      const res = await axios.get(`${API_BASE}/relation/`);
      setAll(res.data);
    } catch (err) {
      console.error(err);
  ...
  const onDelete = async () => {
    ...
    try {
      await axios.delete(`${API_BASE}/relation/delete/${selected.Code}/`);
  ...
  const handleSubmit = async (payload) => {
    try {
      await axios.post(`${API_BASE}/relation/add/`, payload);
  ...
  return (
    <>
      ...
      <div className={`grade-page theme-${theme}`}>
        ...
        <h2 style={{ fontFamily: "-moz-initial" }}> RELATION</h2>
        ...
        <RelationForm
          ...
          onSaved={() => {
            setFormOpen(false);
            loadAll(); // Fetch latest records immediately
          }}

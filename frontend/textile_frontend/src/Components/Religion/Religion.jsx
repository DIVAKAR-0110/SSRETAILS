...
import ReportReligionView from "./ReportReligionView";
...
export default function Religion() {
  ...
  const loadAll = async () => {
    try {
      const res = await axios.get(`${API_BASE}/religion/`);
      setAll(res.data);
    } catch (err) {
      console.error(err);
  ...
  const onDelete = async () => {
    ...
    try {
      await axios.delete(`${API_BASE}/religion/delete/${selected.Code}/`);
  ...
  const handleSubmit = async (payload) => {
    try {
      await axios.post(`${API_BASE}/religion/add/`, payload);
  ...
  return (
    <>
      ...
      <div className={`grade-page theme-${theme}`}>
        ...
        <h2 style={{ fontFamily: "-moz-initial" }}> RELIGION</h2>

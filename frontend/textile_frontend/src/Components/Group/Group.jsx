...
export default function Group() {
  ...
  const loadAll = async () => {
    try {
      const res = await axios.get(`${API_BASE}/group/report/`);
      const data = res.data?.results ?? res.data ?? [];
      setAll(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error(err);
      toast.error("Failed loading groups");
    }
  };
  ...
  useEffect(() => {
    // apply search/filters/order -> paginate client side
    let arr = [...all];
    if (search.trim()) {
  ...
  const onDelete = async () => {
    ...
    try {
      await axios.delete(`${API_BASE}/group/${selected.Code}/`);
  ...
  const handleSubmit = async (payload) => {
    try {
      await axios.post(`${API_BASE}/group/`, payload);
  ...
  return (
    <>
      ...
      <br />
      <br />
      <div className={`group-page theme-${theme}`}>
        <Toaster position="top-right" />
        <h2 style={{ fontFamily: "sans-serif" }}>GROUP</h2>

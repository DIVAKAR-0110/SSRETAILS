...
import ReportGradeView from "./ReportGradeView";
...
export default function Grade() {
  ...
  const loadAll = async () => {
    try {
      const res = await axios.get(`${API_BASE}/grade/report/`);
      const data = res.data ?? res.data.results ?? res.data.data ?? res.data;
  ...
  // apply search/filter/sort -> paginate client-side
  useEffect(() => {
    let arr = [...all];
  ...
  const onDelete = async () => {
    ...
    try {
      await axios.delete(`${API_BASE}/grade/delete/${selected.Code}/`);
  ...
  const onReport = async () => {
    try {
      const res = await axios.get(`${API_BASE}/grade/report/`);
      const data = res.data ?? res.data.results ?? res.data.data ?? res.data;
      setReportData(Array.isArray(data) ? data : data.results ?? []);
      setReportOpen(true);
    } catch (err) {
      console.error(err);
  ...
  const handleSubmit = async (payload) => {
    try {
      // backend supports POST /grade/add/ with optional Code for update
      await axios.post(`${API_BASE}/grade/add/`, payload);
  ...
  return (
    <>
      ...
      <div className={`grade-page theme-${theme}`}>
        ...
        <h2 style={{ fontFamily: "-moz-initial" }}> GRADE</h2>

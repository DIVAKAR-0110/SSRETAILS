...
export default function ReportIncomeExpenseView({
  data = [],
  onClose,
  theme = "metal",
}) {
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = "incomeexpense_report.csv";
    a.click();
              ...
              {filtered.map((r) => (
                <div className="grid-card" key={r.Code}>
                  ...
                  <div className="grid-body">
                    ...
                    <div>Type: {r.HeadType ? "Income" : "Expense"}</div>
                    <div>Acc Head: {r.AccountHeadName}</div>
                    <div>Acc Code: {r.AccountHeadCode}</div>
                    <div>Post: {r.AccountPost ? "Yes" : "No"}</div>
